var Gi=Object.create;var bn=Object.defineProperty;var Ji=Object.getOwnPropertyDescriptor;var Qi=Object.getOwnPropertyNames;var Xi=Object.getPrototypeOf,Zi=Object.prototype.hasOwnProperty;var Oe=(p,m)=>()=>(p&&(m=p(p=0)),m);var Dt=(p,m)=>()=>(m||p((m={exports:{}}).exports,m),m.exports),ki=(p,m)=>{for(var x in m)bn(p,x,{get:m[x],enumerable:!0})},Se=(p,m,x,j)=>{if(m&&typeof m=="object"||typeof m=="function")for(let _ of Qi(m))!Zi.call(p,_)&&_!==x&&bn(p,_,{get:()=>m[_],enumerable:!(j=Ji(m,_))||j.enumerable});return p};var tu=(p,m,x)=>(x=p!=null?Gi(Xi(p)):{},Se(m||!p||!p.__esModule?bn(x,"default",{value:p,enumerable:!0}):x,p)),nu=p=>Se(bn({},"__esModule",{value:!0}),p);var q=Oe(()=>{});var Le={};ki(Le,{createReadStream:()=>ze,createWriteStream:()=>Ne,default:()=>eu,existsSync:()=>Te,lstatSync:()=>Fe,mkdirSync:()=>Ue,readFileSync:()=>De,readdirSync:()=>je,rmdirSync:()=>Pe,statSync:()=>Jn,unlinkSync:()=>Me,writeFileSync:()=>Ie});function Gn(p){return String(p).replace(/[\\]+/g,"/").replace(/^\/+/,"")}function xe(p){var m=Gn(p);if(Rn[m]!==void 0)return Rn[m];for(var x=Object.keys(Rn),j=0;j<x.length;j++)if(m.endsWith("/"+x[j])||m===x[j])return Rn[x[j]]}function Vn(p){var m=Gn(p);if(En[m]!==void 0)return En[m];for(var x=Object.keys(En),j=0;j<x.length;j++)if(m.endsWith("/"+x[j])||m===x[j])return En[x[j]]}var Rn,En,De,je,Te,Ie,Ue,Jn,Fe,Me,Pe,ze,Ne,eu,qe=Oe(()=>{"use strict";q();Rn={"package.json":`{

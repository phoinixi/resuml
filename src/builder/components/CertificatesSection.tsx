    ...
    <Section title={`Certificates (${certificates.length})`}>
      {certificates.map((cert, i) => (
        <div key={i} className="form-entry">
          ...
          <TextInput label="Name" value={(cert['name'] as string | undefined) ?? ''} onChange={(v) => { update(['certificates', i, 'name'], v); }} />
          <TextInput label="Date" value={(cert['date'] as string | undefined) ?? ''} onChange={(v) => { update(['certificates', i, 'date'], v); }} />
          <TextInput label="Issuer" value={(cert['issuer'] as string | undefined) ?? ''} onChange={(v) => { update(['certificates', i, 'issuer'], v); }} />
          <TextInput label="URL" value={(cert['url'] as string | undefined) ?? ''} onChange={(v) => { update(['certificates', i, 'url'], v); }} />
        </div>
      ))}

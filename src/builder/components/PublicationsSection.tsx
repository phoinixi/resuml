    ...
    <Section title={`Publications (${publications.length})`}>
      {publications.map((pub, i) => (
        <div key={i} className="form-entry">
          ...
          <TextInput label="Name" value={(pub['name'] as string | undefined) ?? ''} onChange={(v) => { update(['publications', i, 'name'], v); }} />
          <TextInput label="Publisher" value={(pub['publisher'] as string | undefined) ?? ''} onChange={(v) => { update(['publications', i, 'publisher'], v); }} />
          <TextInput label="Release Date" value={(pub['releaseDate'] as string | undefined) ?? ''} onChange={(v) => { update(['publications', i, 'releaseDate'], v); }} />
          <TextInput label="URL" value={(pub['url'] as string | undefined) ?? ''} onChange={(v) => { update(['publications', i, 'url'], v); }} />
          <TextArea label="Summary" value={(pub['summary'] as string | undefined) ?? ''} onChange={(v) => { update(['publications', i, 'summary'], v); }} />
        </div>
      ))}

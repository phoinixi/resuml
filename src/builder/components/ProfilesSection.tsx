    ...
    <div className="form-field">
      ...
      {profiles.map((profile, i) => (
        <div key={i} className="form-entry">
          ...
          <TextInput label="Network" value={(profile['network'] as string) ?? ''} onChange={(v) => update(['basics', 'profiles', i, 'network'], v)} />
          <TextInput label="Username" value={(profile['username'] as string) ?? ''} onChange={(v) => update(['basics', 'profiles', i, 'username'], v)} />
          <TextInput label="URL" value={(profile['url'] as string) ?? ''} onChange={(v) => update(['basics', 'profiles', i, 'url'], v)} />
        </div>
      ))}

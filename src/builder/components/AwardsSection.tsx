    ...
    <Section title={`Awards (${awards.length})`}>
      {awards.map((award, i) => (
        <div key={i} className="form-entry">
          ...
          <TextInput label="Title" value={(award['title'] as string) ?? ''} onChange={(v) => update(['awards', i, 'title'], v)} />
          <TextInput label="Date" value={(award['date'] as string) ?? ''} onChange={(v) => update(['awards', i, 'date'], v)} />
          <TextInput label="Awarder" value={(award['awarder'] as string) ?? ''} onChange={(v) => update(['awards', i, 'awarder'], v)} />
          <TextArea label="Summary" value={(award['summary'] as string) ?? ''} onChange={(v) => update(['awards', i, 'summary'], v)} />
        </div>
      ))}
